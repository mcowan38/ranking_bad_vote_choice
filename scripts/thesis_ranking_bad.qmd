...
-   The **residual deviance (595.20)** compared to the null deviance (616.32) indicates that the model with race conditions fits the data better than the null model, but still a lot of unexplained variance.
...
-   To statistically compare the rates of Negative Partisanship between Liberals and Conservatives**—**we're directly comparing two proportions here**—**it's suitable to use a non-parametric Fisher's Exact Test**—**particularly useful for small sample sizes or when the expected frequencies in any of the cells of a contingency table are below 5.
...
fishers_test_result <- chisq.test(contingency_table)
...
-   Based on this test, we can conclude that, within the limitations of our sample, there doesn't appear to be a significant difference in the likelihood of engaging in negative partisanship between participants who identify as Conservative and those who identify as Liberal. This suggests either.
